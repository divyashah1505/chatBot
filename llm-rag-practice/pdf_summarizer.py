...
def chunk_text(text: str, max_chars_per_chunk: int = 4000) -> list[str]:
    """
    Splits long text into smaller chunks to fit within LLM context token limit.
    """
...
def summarize_with_llama(text_to_summarize: str, is_chunk: bool = False, chunk_num: int = 1, total_chunks: int = 1) -> str:
    ...
    if is_chunk:
        ...
        prompt = f"Summarize the following section of a document concise bullet points:\n\n{text_to_summarize}"
    else:
...
def summarize_pdf(pdf_path: str) -> str:
    """Main pipeline for PDF summarization (Map-Reduce for large documents)."""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Prompt user or default to example path
