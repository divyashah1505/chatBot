...
1. **Dual AI Engine Architecture (Cloud LLM + Offline Local Fallback)**:
   - Powered by Google Gemini (`gemini-3.6-flash`) for real-time generative intelligence, reasoning, coding, and question answering.
   - Automatically falls back to local LLMs (Ollama / LM Studio) or 100% offline neural TF-IDF engine if offline.
   - Sub-second responses with zero external single point of failure.

2. **Full GitHub-Flavored Markdown & Code Blocks**:
   - Responses formatted like real modern AI (ChatGPT/Gemini).
   - Code cards with language badges and 1-click **"Copy Code"** button.
   - Beautiful data tables, callout notes, and inline code badges.
   - Interactive response actions: **Copy response**, **Read aloud (TTS)**, **Regenerate**, and thumbs up/down rating.
...
│   └── package.json
└── PROJECT_DOCUMENTATION.md     # Complete master documentation
```
